...
async def test_native_router_domain_errors_and_circuit(live_stack):
    ...
    async with httpx.AsyncClient() as http:
        response = await http.get(live_stack.config.gateway.one_url + "/metrics")
        assert "mcp_one_tool_calls_total" in response.text
        status = (await http.get(live_stack.config.gateway.one_url + "/status")).json()
...
async def test_downstream_offline_and_readiness_recovery(live_stack):
    async with Client(live_stack.url, cache=None) as client:
        ...
        try:
            ...
            async with httpx.AsyncClient() as http:
                assert (
                    await http.get(live_stack.config.gateway.one_url + "/health")
                ).status_code == 200
                assert (
                    await http.get(live_stack.config.gateway.one_url + "/ready")
                ).status_code == 503
        finally:
            live_stack.start("mock")
        async with httpx.AsyncClient() as http:
            for _ in range(30):
                await http.post(live_stack.config.gateway.one_url + "/refresh")
                if (
                    await http.get(live_stack.config.gateway.one_url + "/ready")
                ).status_code == 200:
                    break
