...
`demo.echo` remains `demo.echo`. MCP One adds the namespace exactly once and retains the original native
name in its route. `system.echo` under namespace demo becomes
...
original native name remains available inside MCP One for SDK invocation.
